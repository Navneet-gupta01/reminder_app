...
-export([loop/1, init/0, start/0, start_link/0, terminate/0, add_event/3, subscribe/1, add_event2/3, cancel/1, listen/1]).

 %% {list of #event{} records, list of Pids}
...
loop(S = #state{}) ->
  receive
    ...
    {'DOWN', Ref, process, _Pid, _Reason} ->
      io:format("DOWN message: ~n"),
      loop(S#state{clients=orddict:erase(Ref, S#state.clients)});
    code_change ->
      io:format("code_change message: ~n"),
      ?MODULE:loop(S);
    Unknown ->
